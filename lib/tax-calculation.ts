import type {
  ...
  import {
    ...
    ELIGIBLE_FEDERAL_DIVIDEND_TAX_CREDIT,
  ...
  export function calculateSoleProprietorship(
    ...
    // Self-employed can opt into EI, but it's optional - we'll include it for completeness
    const eiContributions = 0 // Optional for self-employed, set to 0 by default
  ...
  export function calculateCorporation(
    ...
    const personalTaxOnSalary = federalTaxOnSalary + provincialTaxOnSalary + employeeCpp + employeeEi + (qpipEmployee || 0) + (healthPremium || 0)
